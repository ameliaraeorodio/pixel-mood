...
function Calendar() {
  ...
  if(localStorage.getItem("data") != null){
    ...
    for (let row = 0; row < ROW_LENGTH; row++) {
      const currRow = [];
      for (let col = 0; col < COL_LENGTH; col++) currRow.push(new Square([row, col, "none", "no entry"]));
      grid.push(currRow);
    }
  }
  ...
  if(!entry) {
          ...
          {grid.map((row) => {
            return (
              <div className={"Row_" + row[0].row}>
                {row.map((square) => {
                  return (
                    <div id = {square.row + " " + square.col} className="square" onClick={() => setEntry(true)}></div>
                  );
                })}
              </div>
            )
    ...
    return (
      <div className="App">
        <button id = "exit" onClick={() => setEntry(false)}>Back</button>
        <br />
...
function someListener(event){
  ...
  if(element.id === "exit"){ load_labels() }
